...
export class ProductionSyncManager {
  ...
  private determineAvailability(
    ...
    if (isPreorderItem && metadata.customAttributes) {
      ...
      const endDate = metadata.customAttributes.preorder_end_date || metadata.preorderCutoffDate;
...
export async function syncAllItemAvailability(): Promise<SyncResult> {
  ...
  try {
    ...
    const items = await db.select().from(menuItems).where(isNull(menuItems.deleted_at));
    ...
    for (const item of items) {
      try {
        ...
        // Update database
        await updateMenuItemAvailability(item.square_item_id, availability, metadata);
  ...
  return (
    <>
      <div className="product-card relative">
        ...
        {/* Product image */}
        <div className="product-image">{/* ... existing image code ... */}</div>
            ...
            <DialogDescription>
              <div className="space-y-4 mt-4">
                ...
                <p className="text-sm text-gray-600">
                  This item is available for pre-order only. Your order will be prepared and
                  delivered after the pre-order period ends.
                </p>
...
describe('Availability Detection', () => {
  ...
  it('hides seasonal items when out of season', () => {
    ...
    const availability = syncManager.determineAvailability(metadata, 'Pride Alfajores');
  ...
  it('respects Square visibility settings', () => {
    ...
    const availability = syncManager.determineAvailability(metadata, 'Test Product');
