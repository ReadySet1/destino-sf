  ...
  if (data.objects) {
    ...
    for (const item of data.objects) {
      if (item.type === 'ITEM' && item.item_data) {
        ...
        const hasInternalVariations =
          item.item_data.variations && item.item_data.variations.length > 1;
...
function determineVariablePriceByItemFullName(itemName: string): number {
  ...
  if (lowerName.includes('plantain')) {
    if (isSmall) return 45.0;
    if (isLarge) return 80.0;
    return 45.0;
  }

  if (lowerName.includes('cheese') && lowerName.includes('charcuterie')) {
    if (isSmall) return 80.0;
    if (isLarge) return 150.0;
    return 80.0;
  }
...
logger.info(`📦 Square items for ${categoryName}:`, {
  ...
  itemsWithVariations: data.objects?.filter((o: any) => o.item_data?.variations?.length > 1).length,
});
```
