...
try {
  ...
  const webhookVars = lines.filter(line => 
  ...
  const hasProductionSecret = lines.some(line => line.includes('SQUARE_WEBHOOK_SECRET') && !line.includes('SANDBOX'));
  ...
  console.log('  SQUARE_WEBHOOK_SECRET_SANDBOX:', hasSandboxSecret ? '✅ Set' : '⚠️  Missing (needed for sandbox)');
