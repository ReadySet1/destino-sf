  ...
  try {
    ...
    try {
      ...
      try {
        ...
        await withRetry(() =>
          prisma.$executeRaw`SELECT 1 as write_test_query`
        );
    ...
    try {
      ...
      if (cateringOrder) {
        console.log(`Found catering order ${cateringOrder.id} with status ${cateringOrder.paymentStatus}`);
    ...
    const hasWriteIssues = results.some(r => 
      (r.test === 'Write Operation' || r.test === 'Webhook Update Simulation') && !r.success
    );
    ...
    if (hasWriteIssues) {
      ...
      console.log('   The webhook issue may be in the application logic rather than database permissions');
    }
