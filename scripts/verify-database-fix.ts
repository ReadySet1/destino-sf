...
async function main() {
  ...
  if (process.env.DATABASE_URL) {
    try {
      ...
      console.log(
        `   Supabase Pooler: ${url.hostname.includes('pooler.supabase.com') ? 'YES' : 'NO'}`
      );
      ...
      console.log(
        `   statement_timeout: ${url.searchParams.get('statement_timeout') || 'not set'}`
      );
    } catch (error) {
  ...
  const averageLatency =
    results.filter(r => r.success).reduce((sum, r) => sum + r.latency, 0) / passed || 0;
