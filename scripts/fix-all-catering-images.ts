 ...
 */
function findBestMatch(cateringItemName: string, products: ProductWithImages[]): ProductWithImages | null {
...
async function fixAllCateringImages() {
  ...
  try {
    ...
    console.log(`  - Items with images: ${withImages}/${verificationItems.length} (${Math.round(withImages/verificationItems.length*100)}%)`);
    console.log(`  - Items with Square links: ${withSquareLinks}/${verificationItems.length} (${Math.round(withSquareLinks/verificationItems.length*100)}%)`);
