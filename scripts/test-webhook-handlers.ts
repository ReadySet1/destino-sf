...
import {
  handleOrderCreated,
  handleOrderUpdated,
  handleOrderFulfillmentUpdated,
} from '../src/lib/webhook-handlers';
...
async function testWebhookHandlers() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        '⚠️ order.created handler error (expected for test data):',
    ...
    try {
      ...
      console.log(
        '⚠️ order.updated handler error (expected for test data):',
        error instanceof Error ? error.message : 'Unknown error'
      );
    }
    ...
    try {
      ...
      console.log(
        '⚠️ order.fulfillment.updated handler error (expected for test data):',
        error instanceof Error ? error.message : 'Unknown error'
      );
    }
