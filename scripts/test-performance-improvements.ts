...
async function testDatabaseIndexes() {
  ...
  const indexTest1 = await measurePerformance('Query products by active status (indexed)', () =>
  ...
  const indexTest3 = await measurePerformance('Query products by creation date (indexed)', () =>
    prisma.product.findMany({
      where: {
        createdAt: {
...
async function testConnectionStability() {
  ...
  for (let i = 0; i < 5; i++) {
    const test = await measurePerformance(`Connection test ${i + 1}`, async () => {
...
async function testBulkOperations() {
  ...
  const bulkTest = await measurePerformance('Bulk update operation (50 products)', async () => {
...
async function runPerformanceTests() {
  ...
  try {
    ...
    logger.info(
      `   • Average connection time: ${connectionResults.averageConnectionTime.toFixed(2)}ms`
    );
