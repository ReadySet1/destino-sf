  ...
  try {
    ...
    for (const order of problematicOrders) {
      ...
      if (paidPayments.length > 0) {
        const latestPayment = paidPayments.sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime())[0];
        console.log(`   Latest PAID payment: ${latestPayment.squarePaymentId} (${latestPayment.updatedAt})`);
    ...
    if (shippingOrders.length > 0) {
      ...
      shippingOrders.forEach(order => {
        console.log(`   - Order ${order.id} (${order.squareOrderId}) - Rate ID: ${order.shippingRateId}`);
      });
