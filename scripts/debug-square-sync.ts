...
async function debugSquareSync() {
  try {
    ...
    logger.info(
      `📊 Found ${items.length} items, ${categories.length} categories, ${images.length} images`
    );
    ...
    for (const item of items) {
      ...
      if (analyzedItems.length <= 5) {
        ...
        logger.info(
          `🖼️  Images: ${analysis.hasImages ? 'YES' : 'NO'} (${analysis.imageIds.length} IDs, ${analysis.imageUrls.length} URLs)`
        );
        ...
        logger.info(
          `💰 Price: ${analysis.hasPrice ? 'YES' : 'NO'} ($${analysis.priceDollars} from ${analysis.priceAmount} cents)`
        );
        logger.info(
          `📁 Category: ${analysis.categoryName || 'NONE'} (ID: ${analysis.categoryId || 'NONE'})`
        );
        ...
        if (analysis.variations.length > 0) {
          ...
          logger.info(
            `   Raw price data: ${JSON.stringify(firstVar.item_variation_data?.price_money)}`
          );
        }
      }
    }
    ...
    logger.info(
      `With images: ${stats.withImages} (${((stats.withImages / stats.totalItems) * 100).toFixed(1)}%)`
    );
    logger.info(
      `Without images: ${stats.withoutImages} (${((stats.withoutImages / stats.totalItems) * 100).toFixed(1)}%)`
    );
    logger.info(
      `With price: ${stats.withPrice} (${((stats.withPrice / stats.totalItems) * 100).toFixed(1)}%)`
    );
    logger.info(
      `Without price: ${stats.withoutPrice} (${((stats.withoutPrice / stats.totalItems) * 100).toFixed(1)}%)`
    );
    logger.info(
      `With category: ${stats.withCategory} (${((stats.withCategory / stats.totalItems) * 100).toFixed(1)}%)`
    );
    ...
    for (const dbProduct of dbProducts.slice(0, 5)) {
      ...
      logger.info(
        `   DB Images: ${dbProduct.images.length} (${dbProduct.images.join(', ').substring(0, 100)}...)`
      );
      ...
      if (squareItem) {
        ...
        // Identify mismatches
        const priceMismatch =
          Math.abs(parseFloat(dbProduct.price.toString()) - squareItem.priceDollars) > 0.01;
        ...
        if (priceMismatch || imageMismatch || categoryMismatch) {
          ...
          if (priceMismatch)
            logger.info(
              `      - Price: DB=$${dbProduct.price} vs Square=$${squareItem.priceDollars}`
            );
          if (imageMismatch)
            logger.info(
              `      - Images: DB=${dbProduct.images.length} vs Square=${squareItem.imageUrls.length}`
            );
          if (categoryMismatch)
            logger.info(
              `      - Category: DB="${dbProduct.category?.name}" vs Square="${squareItem.categoryName}"`
            );
        }
      } else {
    ...
    if (testItem) {
      ...
      logger.info(
        `Fixed image extraction: ${fixedImageIds?.length || 0} image IDs → ${fixedImages.length} URLs`
      );
