...
describe('Error Monitoring System (Phase 2 - Sentry Integration Support)', () => {
  ...
  describe('Configuration Management', () => {
    ...
    it('should use custom critical error threshold', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await customMonitor.captureError(new Error(`Critical error ${i}`), {}, ErrorSeverity.CRITICAL);
      }
  ...
  describe('Specialized Error Capture Methods', () => {
    ...
    it('should capture API errors with request details', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Context: API:POST')
      );
    });
    ...
    it('should capture webhook errors with event details', async () => {
      ...
      await monitor.captureWebhookError(
        webhookError,
        'square',
        'payment.created',
        'evt_123'
    ...
    it('should capture payment errors with transaction details', async () => {
      ...
      await monitor.capturePaymentError(
        paymentError,
        'order-456',
        'pay_123',
        { amount: 7500, cardLast4: '4242' }
      );
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Order: order-456')
      );
    });
  });
  ...
  describe('Error Monitoring Utilities', () => {
    ...
    it('should capture errors from wrapped operations', async () => {
      ...
      await expect(
        withErrorMonitoring(mockOperation, context, mockOnError)
    ...
    it('should handle errors in onError callback', async () => {
      ...
      const mockOperation = jest.fn().mockRejectedValue(operationError);
      const mockOnError = jest.fn().mockImplementation(() => { throw callbackError; });
      
      await expect(
        withErrorMonitoring(mockOperation, {}, mockOnError)
      ).rejects.toThrow('Operation failed');
  ...
  describe('Method Decorator Error Monitoring', () => {
    ...
    it('should simulate decorator functionality with function wrapper', async () => {
      const decoratedFunction = MonitorErrors({ component: 'PaymentService', action: 'chargeCard' });
