...
export function useSeasonalRules(): UseSeasonalRulesReturn {
  ...
  const validateSeasonalConfig = useCallback(
    (config: SeasonalConfig): { isValid: boolean; errors: string[] } => {
      const errors: string[] = [];
      ...
      try {
        ...
        if (
          startDate.getDate() !== config.startDay ||
          startDate.getMonth() !== config.startMonth - 1
        ) {
  ...
  const getNextSeasonDates = useCallback(
    (config: SeasonalConfig, fromDate: Date = new Date()): { start: Date; end: Date } => {
