...
export function CartSummary({ subtotal, totalItems, cartType = 'regular' }: CartSummaryProps) {
  ...
  return (
    <div className={`rounded-lg border p-4 bg-white shadow-sm ${cartType === 'catering' ? 'border-amber-200' : ''}`} data-testid="cart-summary">
      <h2 className={`text-lg font-semibold mb-4 ${cartType === 'catering' ? 'text-amber-700' : ''}`}>
      ...
      <div className="space-y-1">
        ...
        <div className="border-t mt-2 pt-2 font-bold flex justify-between" data-testid="order-total">
          <span>Total</span>
