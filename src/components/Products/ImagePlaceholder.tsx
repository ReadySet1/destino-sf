...
export const ImagePlaceholder: React.FC<ImagePlaceholderProps> = ({
  ...
  const getIcon = () => {
    ...
    if (category === 'dessert' || name.includes('alfajor') || name.includes('cookie') || name.includes('dessert')) {
      return <Cookie className="w-8 h-8 mx-auto" />;
    }
    
    if (category === 'beverage' || name.includes('coffee') || name.includes('tea') || name.includes('drink')) {
  ...
  return (
    ...
    `}>
      <div className="text-center p-2">
        ...
        <span className={`text-xs ${getTextColor()} font-medium leading-tight`}>
          {productName}
        </span>
      </div>
    </div>
