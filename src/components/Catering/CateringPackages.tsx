...
export const CateringPackages: React.FC<CateringPackagesProps> = ({ packages }) => {
  ...
  return (
    <div className="w-full">
      ...
      >
        <h3 className="text-2xl font-bold text-gray-800 mb-4">Prefer to Build Your Own?</h3>
        <p className="text-gray-600 max-w-3xl mx-auto mb-8 text-lg">
          {formatDescription(
            "If our catering packages don't fit your needs, Destino also offers a la carte ordering to create your own custom menu."
          )}
        </p>
        <Link href="/catering/a-la-carte">
          <Button className="bg-[#fab526] hover:bg-[#fab526]/90 text-black font-semibold px-10 py-6 text-lg rounded-full">
      ...
      <div className="text-gray-600 mb-4 space-y-2">
        {sentences.map((sentence, idx) => {
          ...
          if (idx === 0) {
            ...
            return (
              <p key={idx} className="italic text-sm">
                {displaySentence}
              </p>
            );
          } else {
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 h-full flex flex-col">
        <div className="relative">
          ...
          <div className="absolute top-4 right-4 px-4 py-2 bg-white bg-opacity-90 rounded-full text-center shadow-sm">
            <span
              className={
                type === CateringPackageType.INDIVIDUAL ? 'text-blue-700' : 'text-emerald-700'
              }
            >
