  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h2 className="text-3xl font-bold text-gray-900 mb-2">Build Your Own Boxed Lunch</h2>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {tiers.map(tier => (
              <motion.div key={tier.tier} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Card
      ...
      <Card>
        ...
        <CardContent>
          {!selectedTier ? (
            <div className="text-center py-8 text-gray-500">Please select a tier first</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {entrees.map(entree => (
                <motion.div key={entree.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Card
                    className={`cursor-pointer transition-all ${
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                {nameLabel && <p className="text-sm text-gray-600">Label: {nameLabel}</p>}
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold text-orange-600">
                  ${(selectedTier.price * quantity).toFixed(2)}
                </div>
                <div className="text-sm text-gray-600">${selectedTier.price.toFixed(2)} each</div>
              </div>
            </div>
            <Button
