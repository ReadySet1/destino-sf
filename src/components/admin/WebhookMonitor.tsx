...
export function WebhookMonitor({ className, environment }: WebhookMonitorProps) {
  const [selectedEnvironment, setSelectedEnvironment] = useState<SquareEnvironment | undefined>(
    environment
  );
    ...
    <div className={className}>
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <Activity className="w-5 h-5" />
                Webhook Monitor
                <Badge variant={webhookStatus.isHealthy ? 'default' : 'danger'} className="ml-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={e =>
                  setSelectedEnvironment(
                    e.target.value === 'all' ? undefined : (e.target.value as SquareEnvironment)
                  )
                }
              ...
              {/* Manual Sync Button */}
              <Button onClick={handleManualSync} disabled={isSyncing} variant="outline" size="sm">
                {isSyncing ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <StatusCard
          ...
          status={
            webhookStatus.healthScore >= 95
              ? 'excellent'
              : webhookStatus.healthScore >= 90
                ? 'good'
                : webhookStatus.healthScore >= 80
                  ? 'fair'
                  : 'poor'
          }
        ...
        <StatusCard
          ...
          status={
            webhookStatus.successRate > 95
              ? 'excellent'
              : webhookStatus.successRate > 90
                ? 'good'
                : 'poor'
          }
          icon={Activity}
        ...
        <StatusCard
          ...
          status={
            webhookStatus.averageLatency < 200
              ? 'excellent'
              : webhookStatus.averageLatency < 500
                ? 'good'
                : 'poor'
          }
          icon={Clock}
