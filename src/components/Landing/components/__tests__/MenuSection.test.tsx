...
jest.mock('next/image', () => {
  return function MockImage({ src, alt, width, height, priority, unoptimized, fill, sizes, className, loading, ...props }: any) {
    return (
      <img
        src={src}
        alt={alt}
...
describe('MenuSection', () => {
  ...
  describe('Component Rendering', () => {
    ...
    it('renders background pattern with correct number of isotipo images', () => {
      ...
      const backgroundImages = isotipoImages.filter(img => 
        img.getAttribute('src') === '/images/assets/isotipo.png'
      );
  ...
  describe('Responsive Design', () => {
    ...
    it('applies responsive classes to menu item titles', () => {
      ...
      const empanadasTitle = screen.getByText('Our Empanadas');
      expect(empanadasTitle).toHaveClass(
        'text-xl',
        'sm:text-2xl',
        'md:text-3xl'
      );
    ...
    it('applies responsive padding to content container', () => {
      ...
      const contentContainer = screen.getByRole('heading', { name: /menu/i })
        .closest('div')?.querySelector('.mx-auto');
  ...
  describe('Layout and Styling', () => {
    it('applies correct layout classes to menu items container', () => {
      ...
      const menuItemsContainer = screen.getByRole('link', { name: /our empanadas/i })
        .parentElement;
      
      expect(menuItemsContainer).toHaveClass(
  ...
  describe('Content Structure', () => {
    ...
    it('renders background pattern with correct structure', () => {
      ...
      const patternWrapper = screen.getByRole('heading', { name: /menu/i })
        .closest('div')?.querySelector('.patternWrapper');
    ...
    it('applies correct z-index layering', () => {
      ...
      const contentDiv = screen.getByRole('heading', { name: /menu/i })
        .closest('div')?.querySelector('.content');
