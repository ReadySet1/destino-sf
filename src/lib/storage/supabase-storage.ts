...
/**
 * Upload an image to Supabase Storage for catering items
 */
export async function uploadCateringImage(
  ...
  try {
    ...
    const filePath = `catering-items/${fileName}`;
 ...
 */
export async function deleteCateringImage(
  imageUrl: string
...
export async function initializeCateringImagesBucket(): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    if (!bucketExists) {
      ...
      console.log(`Created catering images bucket: ${BUCKET_NAME}`);
    }
