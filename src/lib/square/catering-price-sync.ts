...
export class CateringImageSyncService {
  ...
  async syncImagesAndAvailability(): Promise<CateringImageSyncResult> {
    ...
    try {
      ...
      const allCategories = [...allObjects, ...relatedObjects].filter(
      ...
      for (const squareItem of squareItems) {
        try {
          ...
          logger.info(
            `✅ Updated ${itemName}: ${imageStatus} (${isAvailable ? 'available' : 'unavailable'})`
          );
        } catch (error) {
      ...
      logger.info(
        `✅ Image sync completed: ${result.updated} items updated, ${result.errors.length} errors`
      );
      return result;
    } catch (error) {
  ...
  async getSyncStatus(): Promise<{
    ...
    try {
      ...
      const matchedItems = items.filter(item => item.squareId).length;
  ...
  private extractImageUrl(squareItem: any, relatedObjects: any[]): string | undefined {
    ...
    if (imageIds.length > 0) {
      ...
      const imageObject = relatedObjects.find(
        (obj: any) => obj.type === 'IMAGE' && imageIds.includes(obj.id)
      );
