...
export function formatPhoneForSquare(phone: string): string {
  ...
  if (cleanPhone.startsWith('+')) {
    ...
    if (digitsOnly.length < 7 || digitsOnly.length > 15) {
      throw new Error(
        `Invalid phone number length: ${digitsOnly.length} digits. E.164 format requires 7-15 digits after country code.`
      );
    }
  ...
  if (digitsOnly.length === 10) {
    ...
    throw new Error(
      `Phone number format unclear: ${digitsOnly.length} digits. Please provide phone number with country code (e.g., +1 for US numbers).`
    );
  } else {
    // Invalid length
    throw new Error(
      `Invalid phone number: ${digitsOnly.length} digits. Phone numbers must be 7-15 digits in E.164 format.`
    );
  }
}
...
export function formatPhoneForSquarePaymentLink(phone: string): string | null {
  try {
    ...
    if (digitsOnly.length < 10 || digitsOnly.length > 15) {
      console.warn(
        `Phone number ${formattedPhone} may not be valid for Square payment links (${digitsOnly.length} digits)`
      );
      return null;
    }
    ...
    if (digitsOnly.length === 11 && digitsOnly.startsWith('1')) {
      ...
      if (invalidAreaCodes.includes(areaCode)) {
        console.warn(
          `Phone number ${formattedPhone} has potentially invalid area code ${areaCode} for Square payment links`
        );
        return null;
      }
