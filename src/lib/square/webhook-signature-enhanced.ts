...
function getWebhookSecret(headers: Headers): string | undefined {
  ...
  console.log(`🔐 Square Environment: ${squareEnvironment || 'not specified'} (sandbox: ${isSandbox})`);
...
export async function debugWebhookSignature(
  ...
  try {
    ...
    details.signatureHeader = signatureV2 ? 'x-square-hmacsha256-signature' : 
                             signatureV1 ? 'x-square-signature' : 'none';
    ...
    if (isSandbox && !details.signatureMatch && process.env.SQUARE_WEBHOOK_SECRET) {
      ...
      if (details.fallbackMatch) {
        details.warning = 'Webhook validated with production secret but sent from sandbox environment';
