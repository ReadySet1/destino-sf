 ...
 */
export function constructSquareApiUrl(path: string, environment: 'sandbox' | 'production' = 'production'): string {
  try {
...
export function validateSquareApiResponse(response: any, context: string): void {
  ...
  if (response.errors && response.errors.length > 0) {
    const errorMessages = response.errors.map((err: any) => 
      `${err.code}: ${err.detail || err.message || 'Unknown error'}`
    ).join(', ');
...
export function handleSquareApiError(error: any, context: string): never {
  ...
  if (error.statusCode === 401) {
    throw new Error(`Square API authentication failed in ${context}. Please check your API credentials.`);
  }
  
  if (error.statusCode === 404) {
    throw new Error(`Square API resource not found in ${context}. The requested resource may not exist.`);
  }
  ...
  if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
    throw new Error(`Network error connecting to Square API in ${context}. Please check your internet connection.`);
  }
...
export async function retrySquareApiCall<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      logger.warn(`Square API call attempt ${attempt} failed for ${context}, retrying in ${delayMs}ms:`, error.message);
