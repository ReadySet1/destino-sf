...
export class PaymentSyncService {
  ...
  async syncPayments(request: PaymentSyncRequest): Promise<PaymentSyncResult> {
    ...
    try {
      ...
      console.log(
        `📊 Found ${squarePayments.length} payments in Square from last ${request.lookbackMinutes} minutes`
      );
      ...
      console.log(
        `📋 Analysis: ${existing.length} existing, ${missingIds.length} missing payments`
      );
      ...
      if (ordersWithoutPayments.length > 0) {
        ...
        const orphanResults = await this.processOrphanedOrders(
      ...
      result.success =
        result.errors.length === 0 || result.paymentsProcessed > result.paymentsFailed;
  ...
  private async fetchRecentPayments(lookbackMinutes: number): Promise<any[]> {
    try {
      ...
      throw new Error(
        `Failed to fetch Square payments: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async processSinglePayment(squarePayment: any): Promise<void> {
    ...
    const amount = squarePayment.amount_money?.amount
      ? Number(squarePayment.amount_money.amount) / 100
      : 0;

    // Find the local order
    const localOrder = await withRetry(
      async () => {
