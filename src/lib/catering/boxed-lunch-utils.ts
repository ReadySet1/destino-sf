...
export function extractDietaryInfo(description: string): {
  ...
  const isGlutenFree =
    descLower.includes('-gf') ||
    descLower.includes('gluten free') ||
    descLower.includes('gluten-free');

  const isVegan =
    descLower.includes('-vg') || descLower.includes('vegan') || descLower.includes('-vegan');

  const isVegetarian =
    descLower.includes('vegetarian') || descLower.includes('-vegetarian') || isVegan; // Vegan items are also vegetarian
...
export function createCartItemFromBoxedLunch(
  ...
  const displayName = selectedModifier ? `${item.name} + ${selectedModifier.name}` : item.name;
