...
export function getEffectiveAvailabilityState(product: Product): string {
  ...
  if (product.evaluatedAvailability?.currentState) {
    ...
    if (DEBUG) {
      console.log(`[Availability] ${product.name}: Using evaluated state: ${state} (${product.evaluatedAvailability.appliedRulesCount} rules applied)`);
    }
...
export function canAddToCart(product: Product): boolean {
  ...
  return [
    ...
    AvailabilityState.PRE_ORDER,
  ].includes(state as AvailabilityState);
}
