 ...
 */
export async function findOrdersWithoutPayments(params: {
  since?: Date;
  limit?: number;
}): Promise<Array<{
  id: string;
  ...
  }, 2, 'find-orders-without-payments');
}
...
export async function createPaymentFromSquareData(params: {
  ...
  }, 2, 'create-payment-from-square-data');
}
...
export async function getPaymentSyncMetrics(since?: Date): Promise<{
  ...
  return withRetry(async () => {
    ...
    // Group by sync type
    const syncsByType = syncs.reduce((acc, sync) => {
