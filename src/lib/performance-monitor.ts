...
export class PerformanceMonitor {
  ...
  trackDatabaseQuery(query: string, duration: number, success: boolean = true): void {
    ...
    if (!success) {
      ...
      Sentry.withScope(scope => {
        ...
        Sentry.captureException(new Error(`Failed database query: ${query.substring(0, 100)}`));
      });
    }
  }
  ...
  trackApiPerformance(handler: (req: Request) => Promise<Response>) {
    return async (req: Request) => {
      ...
      try {
        ...
        this.trackApiCall(url.pathname, duration, response.status, req.method);

        return response;
      } catch (error) {
        const duration = Date.now() - start;

        this.trackApiCall(url.pathname, duration, 500, req.method);
  ...
  private cleanupOldMetrics(): void {
    ...
    this.metrics.apiCalls = this.metrics.apiCalls.filter(metric => metric.timestamp > cutoff);
