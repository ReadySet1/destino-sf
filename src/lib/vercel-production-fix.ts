    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => {
          reject(
            new Error(
              `${operationName} timeout after ${config.connectionTimeout / 1000}s (attempt ${attempt})`
            )
          );
      ...
      console.warn(
        `[VERCEL-OPT] ❌ ${operationName} failed on attempt ${attempt}: ${errorMessage}`
      );
...
export async function checkVercelDatabaseHealth(): Promise<{
  ...
  const environment = isVercelProduction() ? 'production' : isVercelPreview() ? 'preview' : 'local';

  try {
    await withVercelOptimization(() => prisma.$queryRaw`SELECT 1 as health_check`, 'health-check');
