...
export function generateCSP(config: CSPConfig): string {
  ...
  const directives: Record<string, string[]> = {
    ...
    'img-src': [
      ...
      ...TRUSTED_DOMAINS.google,
      ...TRUSTED_DOMAINS.other.filter(domain => !domain.includes('upstash') && !domain.includes('resend'))
    ],
 ...
 */
export function getCSPForEnvironment(environment: 'development' | 'production' | 'test', nonce?: string): string {
