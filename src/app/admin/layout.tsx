...
export default function AdminLayout({
  ...
  // --- Dynamic Classes ---
  const sidebarBaseClasses = "fixed inset-y-0 left-0 z-40 w-64 bg-gray-50 border-r transition-transform duration-300 ease-in-out flex flex-col"; // Added flex flex-col
  ...
  return (
    <div className="flex min-h-screen bg-white"> {/* Ensure parent has flex */}
      ...
      >
        {/* Sidebar Content */}
        <div className="p-6 border-b"> {/* Added border-b */}
          <Link href="/" className="text-xl font-bold text-gray-800 hover:text-gray-900">
            Destino SF
          </Link>
        </div>
        <nav className="flex-1 px-4 py-4 overflow-y-auto"> {/* Added flex-1 and overflow */}
          <ul className="space-y-2">
            <NavItem href="/admin" label="Dashboard" icon={<Home size={18} />} currentPath={pathname} />
            <NavItem href="/admin/orders" label="Orders" icon={<ShoppingBag size={18} />} currentPath={pathname} />
            <NavItem href="/admin/products" label="Products" icon={<Archive size={18} />} currentPath={pathname} />
            <NavItem href="/admin/categories" label="Categories" icon={<Tag size={18} />} currentPath={pathname} />
            <NavItem href="/admin/users" label="Users" icon={<Users size={18} />} currentPath={pathname} />
            <NavItem href="/admin/settings" label="Settings" icon={<Settings size={18} />} currentPath={pathname} />
            <NavItem href="/admin/hours" label="Business Hours" icon={<Clock size={18} />} currentPath={pathname} />
          </ul>
        </nav>
        <div className="px-4 py-4 mt-auto border-t"> {/* Added border-t */}
