...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
      ...
      if (mode === 'match' || mode === 'full') {
        try {
          ...
          if (bestMatch) {
            ...
            logger.info(
              `Matched product ${product.name} with Square item ${bestMatch.item_data?.name} (${bestMatch.id})`
            );
            ...
            if (bestMatch.item_data?.image_ids && bestMatch.item_data.image_ids.length > 0) {
              const imageUrls = await getImageUrls(
                bestMatch,
                catalogResponse.result?.related_objects || []
              );
              ...
              if (imageUrls.length > 0) {
                ...
                logger.info(
                  `Updated ${imageUrls.length} images for matched product ${product.name}`
                );
              }
            }
          } else {
    ...
    logger.info(
      `Cleanup complete. Results: ${JSON.stringify({
        total: result.total,
