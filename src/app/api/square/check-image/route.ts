  ...
  try {
    ...
    if (imageId) {
      ...
      try {
        ...
        return NextResponse.json({
          ...
          image: {
            ...
            isSquareS3: originalUrl.includes('items-images-production.s3') || 
        ...
        return NextResponse.json(
          { error: 'Failed to retrieve image', details: error instanceof Error ? error.message : String(error) },
          { status: 500 }
        );
      }
    } else if (objectId) {
      ...
      try {
        ...
        // Process each image
        const images = await Promise.all(imageIds.map(async (imageId: string) => {
          try {
        ...
        return NextResponse.json(
          { error: 'Failed to retrieve item', details: error instanceof Error ? error.message : String(error) },
          { status: 500 }
        );
      }
    }
