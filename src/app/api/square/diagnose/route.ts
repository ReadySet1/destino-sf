...
export async function GET() {
  try {
    ...
    const bestPattern = 
      (clientInfoV1 && clientInfoV1.hasLocationsApi) ? "SquareClient with accessToken" :
      (clientInfoWithToken && clientInfoWithToken.hasLocationsApi) ? "SquareClient with token" :
      (clientInfoV2 && clientInfoV2.hasLocationsApi) ? "Square.Client with accessToken" :
