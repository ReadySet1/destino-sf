...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < productsWithSquareId.length; i += batchSize) {
      ...
      logger.info(
        `Processing batch ${Math.floor(i / batchSize) + 1} of ${Math.ceil(productsWithSquareId.length / batchSize)}`
      );
        ...
        batch.map(async product => {
          try {
            ...
            if (newImages.length > 0) {
              ...
              logger.info(
                `No images found for product "${product.name}" in Square, leaving unchanged`
              );
              results.noChange++;
              results.details.push({
                id: product.id,
    ...
    logger.info(
      `Image refresh complete. Updated: ${results.updated}, No Change: ${results.noChange}, Errors: ${results.errors}`
    );
