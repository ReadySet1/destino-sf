...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.info(`Starting mass product matching with threshold: ${threshold}, updateDb: ${updateDb}`);
    ...
    logger.info(`Found ${products.length} products without Square IDs in the database, processing ${productsToProcess.length}`);
    ...
    logger.info(`Matching complete. Results: ${JSON.stringify({
      total: result.total,
