...
} from '@/lib/square/category-mapper';
...
const UnifiedSyncRequestSchema = z
  .object({
...
async function loadExistingSlugs(): Promise<Set<string>> {
  ...
  existingSlugsCache = new Set(
    products.map(p => p.slug).filter((slug): slug is string => slug !== null)
  );
...
async function getOrCreateCategory(squareId: string, name: string): Promise<any> {
  ...
  category = await prisma.category.findFirst({
    where: {
      OR: [{ squareId }, { name: normalizedName }],
    },
  });
 ...
 */
async function detectSuspiciousCategories(
  allValidSquareIds: string[]
): Promise<Array<{ name: string; expectedProducts: number }>> {
  try {
    ...
    for (const category of categoryStats) {
      if (category.products.length > 0) {
        ...
        const foundInSquare = category.products.filter(
          p => p.squareId && allValidSquareIds.includes(p.squareId)
        ).length;
...
async function getAllActiveSquareProductIds(): Promise<string[]> {
  ...
  try {
    logger.info(
      '🔍 Fetching ALL Square product IDs for archive verification (controlled concurrency)...'
    );
    ...
    const fetchPromises = allCategories.map(([squareId, categoryName]) =>
      limit(async () => {
        try {
          ...
          logger.error(
            `❌ Failed to fetch items for category "${categoryName}" (${squareId}):`,
            error
          );
...
function determineBasePrice(itemName: string): number {
  ...
  if (lowerName.includes('plantain')) {
    return 45.0; // Plantain Chips Platter base price
  } else if (lowerName.includes('cheese') && lowerName.includes('charcuterie')) {
    return 80.0; // Cheese & Charcuterie Platter base price (small)
  } else if (lowerName.includes('cocktail') && lowerName.includes('prawn')) {
    return 80.0; // Cocktail Prawn Platter base price (small)
  }
  ...
  try {
    ...
    for (const config of sharePlatterConfigs) {
      try {
        ...
        logger.info(
          `🔍 DIRECT FIX: Found product "${config.name}" with ${product.variants.length} existing variants`
        );

        // Check if variants are already correct
        const hasCorrectVariants =
        ...
        logger.info(
          `🧹 DIRECT FIX: Cleared ${product.variants.length} existing variants for "${config.name}"`
        );
        ...
        for (const variantConfig of config.variants) {
          ...
          logger.info(
            `✅ DIRECT FIX: Created variant "${variantConfig.name}" for "${config.name}": $${variantConfig.price} (DB ID: ${createdVariant.id})`
          );
        }
        ...
        logger.info(
          `🎯 DIRECT FIX: Successfully fixed "${config.name}" with ${config.variants.length} variants`
        );
      } catch (error) {
    ...
    logger.info(
      `🎯 DIRECT FIX: Completed - fixed ${fixedCount} out of ${sharePlatterConfigs.length} Share Platter items`
    );
    return fixedCount;
  } catch (error) {
  ...
  try {
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        retryCount++;
        const isTimeout =
          apiError?.name === 'TimeoutError' || apiError?.message?.includes('timeout');

        if (isTimeout && retryCount <= maxRetries) {
          const delay = Math.pow(2, retryCount) * 1000; // Exponential backoff: 2s, 4s, 8s
          logger.warn(
            `⏱️ Timeout for category "${categoryName}" (attempt ${retryCount}/${maxRetries}). Retrying in ${delay}ms...`
          );
        ...
        logger.error(
          `❌ Square API error for category "${categoryName}" (${categoryId}) after ${retryCount} attempts:`,
          {
            error: apiError,
            isTimeout,
            requestBody: JSON.stringify(requestBody),
      ...
      for (const item of data.objects) {
        if (item.type === 'ITEM' && item.item_data) {
          ...
          const variations =
            item.item_data.variations?.map((v: any) => {
              ...
              if (v.item_variation_data?.price_money?.amount) {
                ...
                price = determineVariablePriceByName(
                  item.item_data.name,
          ...
          logger.info(`🔍 Extracted variations for "${item.item_data.name}":`, {
            ...
            variations: variations.map((v: { name: string; price: any; id: string }) => ({
              name: v.name,
              price: v.price,
              id: v.id,
            })),
          ...
          const rawDescription =
            item.item_data.description_html || item.item_data.description || '';
    ...
    logger.info(
      `📊 Found ${items.length} items in Square for category "${categoryName}" (${categoryId}):`,
      {
...
async function batchSyncToProducts(
  ...
  try {
    ...
    if (isSharePlattersCategory) {
      ...
      for (const item of items) {
        try {
          ...
          const uniqueSlug = await generateUniqueSlug(
            item.name,
            item.id,
            existingProduct?.slug || undefined
          );
          ...
          for (const variation of item.variations) {
            logger.info(
              `🔨 Creating variant: ${variation.name} with price $${variation.price} (ID: ${variation.id})`
            );
            ...
            logger.info(
              `✅ Successfully created variant "${variation.name}" for ${item.name}: $${variation.price} (DB ID: ${createdVariant.id})`
            );
          }
      ...
      logger.info(
        `📦 Processing regular category "${items[0]?.categoryName}" with bulk operations`
      );
      ...
      try {
        ...
        logger.info(
          `📊 Found ${existingProducts.length} existing products out of ${items.length} items`
        );
          ...
          if (existingProduct) {
            ...
            if (isDuplicate && existingItem) {
              syncLogger.logItemProcessed(
                ...
                item.name,
                'duplicate',
                `Duplicate of existing item: ${existingItem.name}`
              );
              skippedCount++;
            } else {
        ...
        logger.info(
          `📋 Bulk operation plan: ${itemsToCreate.length} creates, ${itemsToUpdate.length} updates`
        );
        ...
        const itemsWithVariations = [...itemsToCreate, ...itemsToUpdate].filter(
          item => item.variations && item.variations.length > 1
        );

        if (itemsWithVariations.length > 0) {
          logger.info(
            `🔧 Processing variants for ${itemsWithVariations.length} items with multiple variations...`
          );
    ...
    logger.info(
      `✅ Batch sync completed: ${syncedCount} synced, ${skippedCount} skipped, ${errorCount} errors`
    );
  ...
  try {
    ...
    if (!category) {
      ...
      try {
        ...
        syncLogger.logInfo(
          `✅ Created new category: ${normalizedCategoryName} (Square ID: ${item.categoryId})`
        );
      } catch (error) {
      ...
      try {
        ...
        syncLogger.logInfo(
          `🔧 Normalized category name: "${category.name}" -> "${normalizedCategoryName}"`
        );
        category.name = normalizedCategoryName; // Update local reference
      } catch (updateError) {
        syncLogger.logInfo(
          `⚠️ Could not update category name (${updateError}), using existing: ${category.name}`
        );
      }
    }
 ...
 */
async function verifySyncCompleteness(syncedItems: SquareItem[]): Promise<SyncVerificationResult> {
...
async function performUnifiedSync(
  ...
  syncLogger.logSyncStart(`Unified Sync (PRODUCTS_ONLY)${dryRun ? ' [DRY RUN]' : ''}`, {
    syncId,
  ...
  try {
    ...
    syncLogger.logInfo(
      `Syncing ${categoriesToSync.length} unique categories (catering + core products)`,
      {
    ...
    const categoryProcessingPromises = categoriesToSync.map(([squareId, localName]) =>
      categoryLimit(async () => {
        ...
        try {
          ...
          syncLogger.logInfo(
            `Found ${squareItems.length} items in Square for ${localName} (${fetchTime}ms)`
          );
          ...
          if (dryRun) {
            ...
            squareItems.forEach(item => {
              syncLogger.logItemProcessed(
                ...
                item.name,
                'synced',
                '[DRY RUN] Would sync to products table'
              );
            });
            ...
            if (localName === 'CATERING- SHARE PLATTERS') {
              ...
              if (fixedVariants > 0) {
                syncLogger.logInfo(
                  `🎯 DIRECT FIX: Successfully applied variants to ${fixedVariants} Share Platter items`
                );
              }
            }
          }
          ...
          syncLogger.logCategoryComplete(
            localName,
            categorySynced,
            categorySkipped,
            categoryErrors
          ...
          if (categoryTotalTime > 30000) {
            // 30 seconds
            syncLogger.logInfo(
              `⚠️ Slow category processing: ${localName} took ${Math.round(categoryTotalTime / 1000)}s`
            );
          }
    ...
    if (!dryRun) {
      ...
      try {
        ...
        syncLogger.logInfo(
          `📊 Archive check will use ${allValidSquareIds.length} total Square product IDs`
        );

        // SAFETY CHECK: Prevent archiving if no Square IDs found (could indicate API failure)
        if (allValidSquareIds.length === 0) {
          syncLogger.logInfo(
            '⚠️ SAFETY CHECK: No Square IDs found - skipping archive to prevent data loss'
          );
          ...
          if (suspiciousCategories.length > 0) {
            ...
            for (const cat of suspiciousCategories) {
              syncLogger.logInfo(
                `   • ${cat.name}: ${cat.expectedProducts} products in DB, but 0 from Square API`
              );
            }
        ...
        syncLogger.logInfo(
          `🗃️ Archive operation: ${archiveResult.archived} products archived, ${archiveResult.errors} errors (${archiveTime}ms)`
        );

        // Performance warning for slow archive operations
        if (archiveTime > 15000) {
          // 15 seconds
    ...
    if (timings.totalSync > 120000) {
      // 2 minutes
      syncLogger.logInfo(`⚠️ SLOW SYNC WARNING: Total sync took ${performanceSummary.totalTime}s`);
    }
    if (timings.squareFetch > 60000) {
      // 1 minute
      syncLogger.logInfo(
        `⚠️ SLOW SQUARE API: Square API calls took ${performanceSummary.squareFetchTime}s`
      );
    }
    if (timings.dbOperations > 45000) {
      // 45 seconds
      syncLogger.logInfo(
        `⚠️ SLOW DATABASE: Database operations took ${performanceSummary.dbOperationsTime}s`
      );
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const {
      strategy: _ignoredStrategy,
      dryRun,
      categories,
      forceUpdate,
      fixSharePlatters,
    } = UnifiedSyncRequestSchema.parse(body);
    ...
    if (_ignoredStrategy && _ignoredStrategy !== 'PRODUCTS_ONLY') {
      logger.warn(
        `🔄 Deprecated strategy '${_ignoredStrategy}' ignored - using PRODUCTS_ONLY (unified data model)`
      );
    }
    ...
    const result = await performUnifiedSync(dryRun, categories, forceUpdate);
...
export async function GET() {
  return NextResponse.json({
    ...
    info: {
      ...
      methods: ['GET', 'POST'],
      description:
        'Comprehensive Square sync for all main product categories - products table only',
      strategy: 'PRODUCTS_ONLY',
      features: [
