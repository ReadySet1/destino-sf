...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // Validate parameters
    if (lookbackMinutes < 1 || lookbackMinutes > 1440) { // Max 24 hours
      return NextResponse.json({
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        successRate: syncResult.paymentsFound > 0 ? 
          Math.round((syncResult.paymentsProcessed / syncResult.paymentsFound) * 10000) / 100 : 100,
        duration: syncResult.duration,
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const { getPaymentSyncHistory, getPaymentSyncMetrics } = await import('@/lib/db/queries/payments');
...
async function validateAdminAuthorization(request: NextRequest): Promise<{
  ...
  try {
    ...
    return {
      valid: false,
      error: 'Missing or invalid authentication. Use x-api-key header or Authorization: Bearer token'
    };
