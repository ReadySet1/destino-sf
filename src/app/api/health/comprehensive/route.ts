...
export async function GET(): Promise<NextResponse<ComprehensiveHealthReport>> {
  ...
  try {
    ...
    const [databaseHealth, paymentsHealth, cacheHealth, emailHealth, businessMetrics, performanceMetrics] = await Promise.all([
      checkDatabaseHealth(),
      checkSquareHealth(),
      checkRedisHealth(),
      checkResendHealth(),
    ...
    const errorReport: ComprehensiveHealthReport = {
      ...
      uptime_seconds: Math.floor(process.uptime()),
      checks: {
        database: { status: 'unhealthy', timestamp: new Date().toISOString(), response_time_ms: 0, error: 'Health check failed' },
        payments: { status: 'unhealthy', timestamp: new Date().toISOString(), response_time_ms: 0, error: 'Health check failed' },
        cache: { status: 'unhealthy', timestamp: new Date().toISOString(), response_time_ms: 0, error: 'Health check failed' },
        email: { status: 'unhealthy', timestamp: new Date().toISOString(), response_time_ms: 0, error: 'Health check failed' }
      },
      business: {
        pending_orders: -1,
        failed_payments: -1,
