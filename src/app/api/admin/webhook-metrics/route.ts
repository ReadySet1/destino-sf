...
export async function GET(request: NextRequest) {
  try {
    ...
    const avgProcessingTimes = processingTimes.reduce((acc, curr) => {
    ...
    return NextResponse.json({
      ...
      health: {
        ...
        failedCount: queueMetrics.statusBreakdown.find(s => s.status === 'FAILED')?._count._all || 0,
...
function calculateSuccessRate(statusBreakdown: Array<{ status: string; _count: { _all: number } }>): string {
