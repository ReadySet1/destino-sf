...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const envHeader =
      request.headers.get('x-square-env') || request.headers.get('square-environment');
    ...
    const detectedEnv =
      envHeader?.toLowerCase() === 'sandbox'
        ? 'sandbox'
        : isDevelopmentHost || isSandboxUserAgent
          ? 'sandbox'
          : 'production';
    ...
    const debugInfo = {
      ...
      secret_selection_logic: {
        for_sandbox: {
          ...
          selected_secret: sandboxSecret
            ? 'SQUARE_WEBHOOK_SECRET_SANDBOX'
            : prodSecret
              ? 'SQUARE_WEBHOOK_SECRET (fallback)'
              : 'NONE',
        },
        for_production: {
    ...
    if (!prodSecret && !sandboxSecret) {
      debugInfo.recommendations.push(
        '⚠️ No webhook secrets configured. Set SQUARE_WEBHOOK_SECRET and SQUARE_WEBHOOK_SECRET_SANDBOX'
      );
    } else if (!sandboxSecret) {
      debugInfo.recommendations.push(
        '⚠️ No sandbox secret. Set SQUARE_WEBHOOK_SECRET_SANDBOX for sandbox webhooks'
      );
    } else if (!prodSecret) {
      debugInfo.recommendations.push(
        '⚠️ No production secret. Set SQUARE_WEBHOOK_SECRET for production webhooks'
      );
    } else {
    ...
    if (
      debugInfo.env_variable_cleaning.whitespace_detected.production ||
      debugInfo.env_variable_cleaning.whitespace_detected.sandbox
    ) {
      debugInfo.recommendations.push(
        '⚠️ Whitespace detected in secrets. Check for trailing newlines in environment variables'
      );
    }
