...
export async function GET(request: NextRequest) {
  try {
    ...
    const isSquareS3Url = imageUrl.includes('items-images-production.s3') || 
    ...
    try {
      ...
      logger.error(`Network error fetching image: ${fetchError instanceof Error ? fetchError.message : String(fetchError)}`);
      return new NextResponse('Network error fetching image', { status: 502 });
    }
    
    if (!response.ok) {
      logger.error(`Error fetching image: ${response.status} ${response.statusText} for URL: ${imageUrl}`);
    ...
    if (!contentType.startsWith('image/')) {
      ...
      return new NextResponse(`The requested resource isn't a valid image for ${imageUrl} received ${contentType}`, { 
 ...
 */
async function tryMultipleSquareFormats(fileId: string, fileName: string): Promise<{
  success: boolean;
  data?: ArrayBuffer;
  contentType: string;
  error?: string;
    ...
    for (const userAgent of userAgents) {
      try {
        ...
        if (response.ok) {
          ...
          logger.warn(`Failed to fetch from ${url} with status ${response.status} ${response.statusText}`);
        }
      } catch (error) {
        // Continue to the next attempt
        logger.warn(`Failed attempt for ${url}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
