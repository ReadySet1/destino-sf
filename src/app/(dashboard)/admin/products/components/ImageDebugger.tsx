      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Input
            ...
            className="flex-1"
          />
          <Button onClick={encodeUrl} variant="outline" size="sm">Encode</Button>
        ...
        {results.length > 0 && (
          <div className="space-y-2 text-sm">
            ...
            {results.map((result, idx) => (
              <div key={idx} className={`p-2 rounded ${result.isImage ? 'bg-green-100' : 'bg-red-100'}`}>
                <p><strong>URL:</strong> {result.url.startsWith('/api') ? 'Proxy URL' : 'Direct URL'}</p>
        ...
        {productImages.length > 0 && (
          <div className="space-y-2 text-sm">
            ...
            {productImages.map((image, idx) => (
              <div key={idx} className={`p-2 rounded ${image.error ? 'bg-red-100' : 'bg-green-100'}`}>
                {image.error ? (
                  <p><strong>Error:</strong> {image.error}</p>
                ) : (
                  <>
                    <p><strong>ID:</strong> {image.id}</p>
                    <p><strong>Name:</strong> {image.name}</p>
                    <p className="text-xs break-all"><strong>URL:</strong> {image.originalUrl}</p>
                    {image.originalUrl && (
                      <div className="mt-2">
