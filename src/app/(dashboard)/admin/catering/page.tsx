...
const AdminCateringPage = async () => {
  ...
  try {
    ...
    if (error instanceof PrismaClientKnownRequestError) {
      if (error.code === 'P2021') {
        errorMessage = 'Catering tables do not exist yet in the database. Using mock data for development.';
      }
    } else {
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ${Math.min(...items.map(i => Number(i.price))).toFixed(0)} - ${Math.max(...items.map(i => Number(i.price))).toFixed(0)}
            </div>
            <p className="text-xs text-muted-foreground">
              Item prices
            </p>
        ...
        <Alert className="mb-6">
          ...
          <AlertDescription>
            ...
            <span className="text-sm text-gray-600">
              This interface shows the same data structure that appears on the customer-facing catering menu.
            </span>
          </AlertDescription>
        </Alert>
      )}
              ...
              {tab === 'all-items' ? (
                ...
                <section>
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold uppercase tracking-wide">Boxed Lunch Packages</h2>
                    <div className="flex gap-2">
                      <BoxedLunchInitializer 
                        hasPackages={packages.filter(pkg => pkg.type === CateringPackageType.BOXED_LUNCH).length > 0} 
                      />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {packages.filter(pkg => pkg.type === CateringPackageType.BOXED_LUNCH).map((pkg) => (
                      <PackageCard key={pkg.id} package={pkg} />
                    ))}
                    {packages.filter(pkg => pkg.type === CateringPackageType.BOXED_LUNCH).length === 0 && (
                ...
                <>
                  ...
                  {packages.filter(pkg => 
                    ...
                    (tab === 'lunch' && pkg.name.toLowerCase().includes('lunch') && pkg.type !== CateringPackageType.BOXED_LUNCH)
                  ).length > 0 && (
                    <section>
                      <div className="flex justify-between items-center mb-6">
                  ...
                  <section>
                    <div className="flex justify-between items-center mb-6">
                      <h2 className="text-2xl font-bold uppercase tracking-wide">Individual Items</h2>
                      <Button asChild size="sm" variant="outline">
...
const PackageCard = ({ package: pkg }: { package: CateringPackage }) => {
  const averageRating = pkg.ratings && pkg.ratings.length > 0 
