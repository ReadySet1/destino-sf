...
function loadBackupMetadata(backupFile: string): BackupMetadata | null {
  ...
  if (fs.existsSync(metadataPath)) {
    try {
      ...
      console.warn(
        '⚠️  Failed to load backup metadata:',
...
function verifyBackupFile(backupFile: string): void {
  ...
  if (stats.isDirectory()) {
    ...
    console.log(
      `✅ Directory backup verified (${formatFileSize(calculateDirectorySize(backupFile))})`
    );
  } else {
...
async function createDatabase(dbInfo: DatabaseConnectionInfo, targetDb: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      `Failed to create database ${targetDb}: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
...
async function dropDatabase(dbInfo: DatabaseConnectionInfo, targetDb: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      `Failed to drop database ${targetDb}: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
  ...
  try {
    ...
    if (!config.dryRun) {
      ...
      if (config.createDatabase || (config.dropExisting && dbExists)) {
        ...
        throw new Error(
          `Target database '${targetDb}' does not exist. Use --create-db to create it.`
        );
      }
    }
