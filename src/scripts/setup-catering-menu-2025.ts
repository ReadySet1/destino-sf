...
/**
 * Script to set up the 2025 Catering Menu with proper packages and pricing
...
async function setupCateringMenu() {
  console.log('🚀 Setting up 2025 Catering Menu...');
  ...
  try {
    ...
    console.log(`   • Platter Items: ${PLATTER_ITEMS.length} processed`);
    console.log(`   • Dessert Items: ${DESSERT_ITEMS.length} processed`);
    console.log(`   • Packages: ${createdPackages} created, ${updatedPackages} updated`);
    console.log(`   • Business Rules: 5-day advance notice implemented`);
  } catch (error) {
...
async function main() {
  try {
    await setupCateringMenu();
  } catch (error) {
    console.error('Failed to setup catering menu:', error);
    process.exit(1);
  } finally {
