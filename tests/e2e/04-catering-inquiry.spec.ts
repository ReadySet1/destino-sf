...
test.describe('Catering Inquiry & Quote', () => {
  ...
  test('should display catering menu items with prices', async ({ page }) => {
    ...
    await expect(page.locator('div').filter({ hasText: /Classic Alfajores/ }).locator('div').filter({ hasText: /\$2\.50/ })).toBeVisible();
  });
  ...
  test('should submit catering contact form', async ({ page }) => {
    ...
    await page.fill('textarea[placeholder="Your message"]', 
